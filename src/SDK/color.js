...
class Color {
  ...
  rgb_to_hex(r, g, b) {
    [r, g, b].map(this.checkInitValue)
    return `#${[r, g, b].map(this.stringRgbToHex).join(' ').replace(/\s*/g, '')}`
  }
  ...
  hex_to_rgb(hex) {
    ...
    const h = hex.substring(1, 3),
      e = hex.substring(3, 5),
      x = hex.substring(5, 7);
    const rgb = [h, e, x].map(this.numberHexToRgb)
    return `rgb(${rgb.join()})`
  }
